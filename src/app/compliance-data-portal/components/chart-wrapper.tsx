...
const ChartWrapper = forwardRef<HTMLDivElement, Props>(
  (
    {
      title,
      tabs,
    ...
    const [expanded, setExpanded] = useState<boolean[]>(
      addons?.map((addon) => addon.defaultExpanded ?? true) ?? []
    );
    ...
    return (
      <div className="w-full" ref={ref} {...props}>
        <Card>
            ...
            <CardTitle className="flex w-full flex-col gap-2 md:flex-row flex-wrap justify-between">
              ...
              <div className="flex flex-col md:flex-row justify-end gap-2">
                ...
                {dataTabs && currentDataTab && setCurrentDataTab && (
                  <TabsSelector
                    tabs={dataTabs}
                    currentTab={currentDataTab}
                    setCurrentTab={setCurrentDataTab}
                  />
                )}
              </div>
            </CardTitle>
          ...
          <CardContent>
            {!!addons?.length && (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-y-2 sm:gap-2 mb-6">
                {addons?.map((addon, index) => {
                  ...
                  return (
                    <Card
                      alternate
                      key={index}
                      style={{ gridColumn: `span ${addon.size ?? 1}` }}
                    >
                      <CardHeader>
                        <CardTitle className="w-full flex items-center justify-between">
                          ...
                          {!isDesktop && addon.expandable && (
                            <Drawer>
                              ...
                              <DrawerContent>
                                <div className="p-4">{addon.value}</div>
                              </DrawerContent>
                            </Drawer>
                          )}
                        </CardTitle>
