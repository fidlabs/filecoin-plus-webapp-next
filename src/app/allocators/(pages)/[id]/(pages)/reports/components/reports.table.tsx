...
const ReportsTable = ({reportsData}: IProps) => {
  ...
  const selectedIds = useMemo(() => {
    return Object.keys(rowSelection).map((key) => reportsData?.[+key]?.id).filter(item => !!item) as string[]
  ...
  return <div>
    ...
    {selectedIds?.length > 1 && <CardFooter className="border-t w-full p-3 justify-between">
      ...
      <Link href={`reports/${selectedIds?.sort((a, b) => +a - +b)?.join('/')}`} className={buttonVariants({variant: "outline"})}>
        Compare
      </Link>
    </CardFooter>}
  </div>
