"use client";
import {
  ICDPAllocatorFullReport, ICDPAllocatorFullReportClient, ICDPAllocatorFullReportStorageProviderDistribution,
...
const ReportsDetailsProvider = ({children, reports}: PropsWithChildren<{ reports: ICDPAllocatorFullReport[] }>) => {
  ...
  const providersDistribution = useMemo(() => reports.map(report => report.storage_provider_distribution), [reports])

  const mapsConstraints = useMemo(() => {
    const locations = providersDistribution.flatMap(providerList => providerList.filter(provider => !provider.not_found && !!provider.location).map(provider => provider.location));
