...
const ReportDetail = async ({params}: IPageProps) => {
  ...
  const reports = await Promise.allSettled(params.report.map(async (report) => getAllocatorReportById(params.id, report)))
  return <ReportsDetailsProvider
    reports={parseReports(reports as PromiseFulfilledResult<ICDPAllocatorFullReport>[])}>
    <ReportsLayout/>
  </ReportsDetailsProvider>
}
