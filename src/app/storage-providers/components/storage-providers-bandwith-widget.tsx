...
interface StorageProvidersBandwidthWidgetProps extends CardProps {
...
function getLabelForRange(range: Range): string {
  ...
  if (lowerBound === null) {
    return upperBound === 0 ? "0 Mbps" : `Less than ${upperBound} Mbps`;
  }

  if (upperBound === null) {
    return `Over ${lowerBound} Mbps`;
  }

  return `${lowerBound} Mbps - ${upperBound} Mbps`;
}
...
export function StorageProvidersBandwidthWidget({
  animationDuration = 500,
  className,
  ...rest
}: StorageProvidersBandwidthWidgetProps) {
  ...
  return (
    <Card {...rest} className={cn("pb-4", className)}>
      <header className="px-4 py-4 max-w-[min(50vw, 200px)]">
        <h3 className="text-lg font-medium">Bandwidth Histogram</h3>
        <p className="text-xs text-muted-foreground">
          Storage Providers and their Datacap grouped by their Bandwidth
          measurment.
        </p>
      </header>
