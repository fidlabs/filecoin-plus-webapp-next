...
const Component = ({ data, allocators }: Props) => {
  ...
  const clickLegend = useCallback(
    (event: MouseEvent<HTMLButtonElement>, entry: string) => {
      ...
      if (event.shiftKey) {
        if (!selectedValues.length) {
          ...
          const lastIndex = valueKeys.indexOf(
          ...
          const start = Math.min(
            firstIndex,
            selectedIndex,
            selectedValues.indexOf(entry) > -1 ? selectedIndex : lastIndex
          );
          const end = Math.max(
            firstIndex,
            selectedIndex,
        ...
        setSelectedValues((curr) =>
          curr.indexOf(entry) > -1
            ? curr.filter((item) => item !== entry)
            : [...curr, entry]
        );
      } else {
        setSelectedValues((curr) =>
          curr.length === 1 && curr.indexOf(entry) > -1
            ? curr.filter((item) => item !== entry)
            : [entry]
        );
      }
  ...
  const filteredData = useMemo(() => {
    return chartData.filter((item) =>
      valuesToDisplay.some((key) => Object.keys(item).includes(key))
    );
    ...
    <Card className="hidden md:block md:col-span-3">
      <CardHeader>
        ...
        <div className="flex gap-2">
          <Tabs
            value={mode}
            onValueChange={(val) => setMode(val as "week" | "allocator")}
          >
            <TabsList>
      ...
      <CardContent className="flex items-center justify-center">
            ...
            >
              ...
              {valuesToDisplay.map((key) => (
                <Bar
                  ...
                  maxBarSize={50}
                  stackId="a"
                  fill={palette(valueKeys.indexOf(key))}
                />
              ))}
            </BarChart>
          </ResponsiveContainer>
        ...
        >
          {reversedValueKeys.map((entry, index) => {
            const allocator =
              mode === "allocator"
                ? allocators.data.find(
                    (allocator) => allocator.addressId === entry
                  )
                : undefined;
            const name =
              mode === "allocator"
                ? (getAllocatorName(allocator) ?? entry)
                : `Week ${entry.substring(1)}`;
            return (
              <button
                className={cn(
                  "bg-transparent text-xs flex items-center rounded-md h-6 p-1 hover:grayscale-0 hover:opacity-100 hover:bg-solitude",
                  { ["grayscale-[80%] opacity-60"]: !!selectedValues.length },
                  {
                    ["!bg-oyster-bay grayscale-0 opacity-100 active"]:
                      selectedValues.indexOf(entry) > -1,
                  }
                )}
                onClick={(e) => clickLegend(e, entry)}
                key={`item-${index}`}
              >
                <div
                  className={"w-5 h-full mr-1 rounded"}
                  style={{ backgroundColor: palette(valueKeys.indexOf(entry)) }}
                />
