...
export type ClientsStatisticsWidgetProps = Omit<
  HTMLAttributes<HTMLDivElement>,
  "children"
>;
type Interval = NonNullable<
  FetchClientsDashboardStatisticsParameters["interval"]
>;

export function ClientsStatisticsWidget(props: ClientsStatisticsWidgetProps) {
  const [interval, setIntervalValue] = useState<Interval>("day");
  const { data, error, isLoading, mutate } = useSWR(
    [QueryKey.CLIENTS_DASHBOARD_STATISTICS, { interval }],
    ([, fetchParams]) => {
      return fetchClientsDashboardStatistics(fetchParams);
    },
