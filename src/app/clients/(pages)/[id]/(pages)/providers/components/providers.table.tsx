...
const ProvidersTable = () => {
  const { providersData, activeProviderIndex, setActiveProviderIndex } =
    useClientProvidersDetails();

  return (
    <div className="overflow-auto md:max-h-[calc(100vh-400px)]">
      <Table>
        <TableHeader>
          <TableCell>Provider</TableCell>
          <TableCell>% of total client used DataCap</TableCell>
          ...
          {providersData?.stats.map(
            ({ provider, percent, total_deal_size }, index) => (
              ...
              >
                <TableCell>
                  <Link
                    href={`/storage-providers/${provider}`}
                ...
                <TableCell>{convertBytesToIEC(total_deal_size)}</TableCell>
              </TableRow>
            )
          )}
        </TableBody>
