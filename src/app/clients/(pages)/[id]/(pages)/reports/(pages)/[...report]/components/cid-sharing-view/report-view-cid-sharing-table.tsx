"use client";
import { IClientReportCIDSharing } from "@/lib/interfaces/cdp/cdp.interface";
  ...
  const columns = [
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <Link className="table-link" href={`/clients/${other_client}`}>
            {other_client}
          </Link>
        );
      },
    },
    {
      ...
      header: () => {
        return <div className="whitespace-nowrap">Total deals affected</div>;
      },
      cell: ({ row }) => {
    ...
    },
    {
      ...
      header: () => {
        return <div className="whitespace-nowrap">Unique CIDs</div>;
      },
      cell: ({ row }) => {
        if (row.original.not_found) {
          return (
        ...
        return (
          <div className="h-full flex items-center justify-end gap-1">
            {unique_cid_count}
            {compareMode && (
              <CompareIcon compare={row.original.unique_cid_count_compare} />
            )}
          </div>
        );
      },
...
const ReportViewCidSharingTable = ({
  cidSharingData,
}: IReportViewCidSharingTable) => {
  const { compareMode } = useReportsDetails();

  const { columns } = useReportViewCidSharingColumns(compareMode);
  ...
  const rowSelection = useMemo(() => {
    ...
    for (let i = 0; i < cidSharingData.length; i++) {
      ...
      selection[i] =
        comparableValues.includes(item.unique_cid_count_compare ?? "equal") ||
