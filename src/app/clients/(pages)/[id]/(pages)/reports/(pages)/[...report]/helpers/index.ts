...
export const parseReport = (result: PromiseFulfilledResult<IClientFullReport>, allProviders: IClientReportStorageProviderDistribution[], allReplikas: IClientReportReplicaDistribution[], allCidShares: IClientReportCIDSharing[]) => {
  ...
  value.replica_distribution = [
    ...allReplikas.filter(replica => !value.replica_distribution.find(r => r.num_of_replicas === replica.num_of_replicas)),
    ...value.replica_distribution
  ].sort((a, b) => +a.num_of_replicas - +b.num_of_replicas)

  value.cid_sharing = [
    ...allCidShares.filter(cidShare => !value.cid_sharing.find(c => c.other_client === cidShare.other_client)),
    ...value.cid_sharing
  ...
  return reports.map((report, index) => {
    ...
    return {
      ...
      storage_provider_distribution: report.storage_provider_distribution.map((provider, providerIndex) => {
        return {
          ...provider,
          total_deal_size_compare: compare(+provider.total_deal_size, +reports[index - 1].storage_provider_distribution[providerIndex].total_deal_size),
          total_deal_percentage_compare: compare(provider.total_deal_percentage, reports[index - 1].storage_provider_distribution[providerIndex].total_deal_percentage),
      ...
      replica_distribution: report.replica_distribution.map((replica, replicaIndex) => {
        return {
          ...replica,
          total_deal_size_compare: compare(+replica.total_deal_size, +reports[index - 1].replica_distribution[replicaIndex].total_deal_size),
          unique_data_size_compare: compare(+replica.unique_data_size, +reports[index - 1].replica_distribution[replicaIndex].unique_data_size),
          percentage_compare: compare(replica.percentage, reports[index - 1].replica_distribution[replicaIndex].percentage),
        }
      }),
      cid_sharing: report.cid_sharing.map((cidShare, cidShareIndex) => {
        return {
          ...cidShare,
          total_deal_size_compare: compare(+cidShare.total_deal_size, +reports[index - 1].cid_sharing[cidShareIndex].total_deal_size),
          unique_cid_count_compare: compare(cidShare.unique_cid_count, reports[index - 1].cid_sharing[cidShareIndex].unique_cid_count),
        }
      }),
    }
  })
...
export const parseReports = (reports: PromiseFulfilledResult<IClientFullReport>[]) => {
  ...
  const allReplikas = prepareEmptyReplicas(reports.map(report => report.value.replica_distribution.map(provider => provider.num_of_replicas)).flat())
  const allCidShares = prepareEmptyCIDSharing(reports.map(report => report.value.cid_sharing.map(provider => provider.other_client)).flat())
  const richReports = reports.map((report) => parseReport(report, allProviders, allReplikas, allCidShares))
