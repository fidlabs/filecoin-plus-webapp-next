  ...
  const columns = [
    {
      ...
      header: () => {
        return <div className="whitespace-nowrap">Providers</div>;
      },
      cell: ({ row }) => {
    ...
    },
    {
      ...
      header: () => {
        return <div className="whitespace-nowrap">Unique Data</div>;
      },
      cell: ({ row }) => {
        ...
        return (
          <div className="h-full flex items-center justify-start gap-1">
            ...
            {compareMode && (
              <CompareIcon compare={row.original.unique_data_size_compare} />
            )}
          </div>
        );
      },
    },
    {
      ...
      header: () => {
        return <div className="whitespace-nowrap">Total Deals</div>;
      },
      cell: ({ row }) => {
        if (row.original.not_found) {
          return (
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="h-full flex items-center justify-end gap-1">
            {percentage}%
            {compareMode && (
              <CompareIcon compare={row.original.percentage_compare} />
            )}
          </div>
        );
      },
...
const ReportViewReplicaTable = ({ replikaData }: IReportViewReplicaTable) => {
  ...
  const { columns } = useReportViewReplikasColumns(compareMode);
  ...
  const rowSelection = useMemo(() => {
    ...
    for (let i = 0; i < replikaData.length; i++) {
      ...
      selection[i] =
        comparableValues.includes(item.unique_data_size_compare ?? "equal") ||
        comparableValues.includes(item.total_deal_size_compare ?? "equal") ||
        comparableValues.includes(item.percentage_compare ?? "equal");
    }
