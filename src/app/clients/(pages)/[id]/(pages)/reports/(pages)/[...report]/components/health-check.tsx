"use client";
import { IClientReportCheckResult } from "@/lib/interfaces/cdp/cdp.interface";
...
const defaultMessages = {
  ...
  DEAL_DATA_REPLICATION_LOW_REPLICA: (status: boolean) =>
    status ? "Low replica percentage" : "High replica percentage",
  DEAL_DATA_REPLICATION_CID_SHARING: (status: boolean) =>
    status
      ...
      {security.map((item, index) => {
        return (
          ...
          >
            ...
            {item?.metadata?.msg ??
              getDefaultMessage(item.check as DefaultMessageKeys, item.result)}
          </div>
        );
      })}
    </div>
