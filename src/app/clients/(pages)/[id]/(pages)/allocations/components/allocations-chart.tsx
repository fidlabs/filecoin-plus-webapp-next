...
const AllocationsChart = ({ allocationsData }: IProps) => {
  const renderTooltip = (props: TooltipProps<ValueType, NameType>) => {
    ...
    return (
      <Card>
        <CardHeader>
          <CardTitle>
            {calculateDateFromHeight(allocationData["height"])}
          </CardTitle>
        </CardHeader>
        <CardContent>
          ...
          >{`Allocated: ${convertBytesToIEC(allocationData["allowance"])}`}</p>
          <p
            style={{ color: palette(64) }}
          >{`Total: ${convertBytesToIEC(allocationData["totalAllowance"])}`}</p>
        </CardContent>
      </Card>
    );
  };
  ...
  const chartData = useMemo(() => {
    ...
    const flatAllowanceArray = allocationsData?.data?.flatMap(
      (item) => item.allowanceArray
    );
    ...
    flatAllowanceArray
      ...
      .forEach((item) => {
        returnData.push({
          ...
          totalAllowance:
            returnData.reduce((acc, cur) => acc + +cur.allowance, 0) +
            +item.allowance,
        });
      });
  ...
  return (
    <div className="lg:max-h-[50vh] overflow-y-auto overflow-x-hidden">
      {chartData && (
        <ResponsiveContainer
          width="100%"
          height="100%"
          aspect={isDesktop ? 2.7 : 1}
          debounce={500}
        >
          <ComposedChart
            width={500}
