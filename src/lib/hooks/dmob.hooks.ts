...
const useDataCapFlow = () => {
  ...
  const getElement = useCallback(
    (
      nodeIdGenerator: Generator<number>,
      name: string,
      array: IAllocatorWithSheetInfo[],
      withSimpleChildren = false
    ) => {
      const element = {
        name,
  ...
  const getAudits = useCallback(
    (nodeIdGenerator: Generator<number>, results: IAllocatorsWithSheetInfo) => {
          ...
          children: !!notWaitingData.length
            ? [
                getElement(
                  nodeIdGenerator,
                  "Failed",
                  notWaitingData.filter(failedFilter(i)),
                  true
                ),
                getElement(
                  nodeIdGenerator,
                  "Conditional",
                  notWaitingData.filter(partialFilter(i)),
                  true
                ),
                getElement(
                  nodeIdGenerator,
                  "Pass",
                  notWaitingData.filter(passFilter(i)),
                  true
                ),
              ]
            : undefined,
    ...
    return [
      {
        ...
        children: [
          getElement(
            nodeIdGenerator,
            "Not Active",
            results.data.filter(notActiveFilter),
            true
          ),
          {
            ...getElement(
              nodeIdGenerator,
              "Active",
              results.data.filter(activeFilter)
            ),
