...
const useGoogleSheetFilters = () => {
  ...
  const activeFilter = useCallback(
    (data: IAllocatorWithSheetInfo) => data.isActive,
    []
  );
  const notActiveFilter = useCallback(
    (data: IAllocatorWithSheetInfo) => !data.isActive,
    []
  );

  const notAuditedFilter = useCallback(
    (data: IAllocatorWithSheetInfo) =>
      activeFilter(data) && data.isAudited === false,
    [activeFilter]
  );
  const auditedFilter = useCallback(
    (data: IAllocatorWithSheetInfo) =>
      activeFilter(data) && data.isAudited === true,
    [activeFilter]
  );

  const notWaitingFilter = useCallback(
    (i: number) => (data: IAllocatorWithSheetInfo) =>
      auditedFilter(data) &&
      !!data.auditStatuses[i] &&
      data.lastValidAudit === i &&
      data.auditStatuses[i] !== "WAITING",
    [auditedFilter]
  );

  const failedFilter = useCallback(
    (i: number) => (data: IAllocatorWithSheetInfo) =>
      notWaitingFilter(i)(data) &&
      !!data.auditStatuses[i] &&
      FAILED_STATUSES.includes(data.auditStatuses[i]),
    [FAILED_STATUSES, notWaitingFilter]
  );
  const partialFilter = useCallback(
    (i: number) => (data: IAllocatorWithSheetInfo) =>
      notWaitingFilter(i)(data) &&
      !!data.auditStatuses[i] &&
      PARTIAL_STATUSES.includes(data.auditStatuses[i]),
    [PARTIAL_STATUSES, notWaitingFilter]
  );
  const passFilter = useCallback(
    (i: number) => (data: IAllocatorWithSheetInfo) =>
      notWaitingFilter(i)(data) &&
      !!data.auditStatuses[i] &&
      PASS_STATUSES.includes(data.auditStatuses[i]),
    [PASS_STATUSES, notWaitingFilter]
  );
...
const useGoogleSheetsAuditReport = () => {
  ...
  const [googleSheetsAuditHistory, setGoogleSheetsAuditHistory] = useState<
    IGoogleSheetResponse | undefined
  >(undefined);
  const [googleSheetsAuditSizes, setGoogleSheetsAuditSizes] = useState<
    IGoogleSheetResponse | undefined
  >(undefined);
  ...
  const parsedData = useMemo(() => {
    ...
    if (
      data?.data &&
      googleSheetsAuditHistory?.values &&
      googleSheetsAuditSizes?.values
    ) {
      const allocatorIdIndex =
        googleSheetsAuditHistory.values[0].indexOf("Allocator ID");
      const allocatorIdName =
        googleSheetsAuditHistory.values[0].indexOf("Allocator");
      const firstReviewIndex = googleSheetsAuditHistory.values[0].indexOf("1");
      const firstReviewIndexSizes =
        googleSheetsAuditSizes.values[0].indexOf("1");

      returnData.audits =
        +googleSheetsAuditHistory.values[0][
          googleSheetsAuditHistory.values[0].length - 1
        ];
      ...
      data?.data.forEach((result) => {
        const googleAuditHistoryData = googleSheetsAuditHistory?.values.find(
          (data) => data[allocatorIdIndex] === result.addressId
        );
        const googleAuditSizesData = googleSheetsAuditSizes?.values.find(
          (data) => data[allocatorIdIndex] === result.addressId
        );
        const auditStatuses = googleAuditHistoryData
          ? googleAuditHistoryData
              .slice(firstReviewIndex)
              .map((item) => item.toUpperCase())
          : [];
        const auditSizes = googleAuditSizesData
          ? googleAuditSizesData.slice(firstReviewIndexSizes).map((item) => {
              const numeric = +item;
        ...
        const name =
          googleAuditHistoryData?.[allocatorIdName] ?? result.orgName;
        ...
        if (!!name) {
          returnData.data.push({
            ...
            isActive:
              (googleAuditHistoryData &&
                googleAuditHistoryData[firstReviewIndex] !== "Inactive") ||
              false,
            isAudited:
              googleAuditHistoryData &&
              !["Inactive", "Waiting", "Pre Audit"].includes(
                googleAuditHistoryData[firstReviewIndex]
              ),
            lastValidAudit:
              auditStatuses.filter(
                (item) => !["WAITING", "PRE AUDIT", "INACTIVE"].includes(item)
              ).length - 1,
          });
        }
      });
...
const useGoogleTrustLevels = (scale: string, mode: string) => {
  ...
  const parsedData = useMemo(() => {
    ...
    for (let i = _firstMonthInxed; i <= _lastMonthIndex; i++) {
      ...
      const unknown = isAllocator
        ? unknownList.length
        : unknownList.reduce(
            (acc, row) => acc + +(row[i].split("-")?.[1] ?? 0),
            0
          ) * PIB;
      ...
      const nonCompliant = isAllocator
        ? nonCompliantList.length
        : nonCompliantList.reduce(
            (acc, row) => acc + +(row[i].split("-")?.[1] ?? 0),
            0
          ) * PIB;
      ...
      const likelyCompliant = isAllocator
        ? likelyCompliantList.length
        : likelyCompliantList.reduce(
            (acc, row) => acc + +(row[i].split("-")?.[1] ?? 0),
            0
          ) * PIB;
      ...
      const compliant = isAllocator
        ? compliantList.length
        : compliantList.reduce(
            (acc, row) => acc + +(row[i].split("-")?.[1] ?? 0),
            0
          ) * PIB;
      ...
      returnData.push({
        ...
        likelyCompliant: isPercent
          ? (likelyCompliant / total) * 100
          : likelyCompliant,
...
const useAuditTimeline = (scale: string) => {
  ...
  const parsedData = useMemo(() => {
    ...
    const dataArray = rows
      .filter((item) => item[_outcomeIndex].toLowerCase() !== "waiting")
      .map((item) => ({
    ...
    Object.values(groupedArray).forEach((children) => {
      ...
      const avgConversationLength =
        children.reduce((acc, row) => acc + +(row.conversationLength || 0), 0) /
        children.length;
      const avgAuditTime =
        children.reduce((acc, row) => acc + +(row.auditTime || 0), 0) /
        children.length;
      ...
      returnData.push({
        name: reviewName,
        avgConversationLength: isPercent
          ? Math.round((avgConversationLength / total) * 100)
          : Math.round(avgConversationLength),
        avgConversationLengthName: "Average Conversation Length",
        avgAuditTime: isPercent
          ? Math.round((avgAuditTime / total) * 100)
          : Math.round(avgAuditTime),
...
export {
  useGoogleSheetsAuditReport,
  useGoogleSheetFilters,
  useGoogleTrustLevels,
  useAuditTimeline,
};
