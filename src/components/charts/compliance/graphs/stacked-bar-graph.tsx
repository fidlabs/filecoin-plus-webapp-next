...
const StackedBarGraph = ({
      ...
      <Card>
        ...
        <CardContent>{
          dataKeys.map((key: string, index: number) => {
            ...
            const valueText = usePercentage || currentDataTab === 'Count' ? value : convertBytesToIEC(value);
            return <div key={key} className="chartTooltipRow">
              {!usePercentage && <div style={{color}}>{name} - {valueText} {unit}{value !== 1 && 's'}</div>}
              {usePercentage && <div style={{color}}>{name} - {value.toFixed(2)}% of {unit}s</div>}
            </div>
          })
        }
          {payload['avgSuccessRate'] &&
            <div className="mt-2 text-sm text-[#252525]">Average Success Rate - {payload['avgSuccessRate'].toFixed(2)}%</div>}
        </CardContent>
      </Card>
    );
  };
  ...
  const dataKeys = useMemo(() => {
    return uniq(data.flatMap(data => Object.keys(data)).filter(key => !key.toLowerCase().includes('name') && !key.includes('avgSuccessRate')));
  }, [data]);
  ...
  return <div>
    <ResponsiveContainer width="100%" aspect={isDesktop ? 1.8 : 0.5} debounce={500}>
      <BarChart
        ...
        data={data}
        margin={{bottom: isDesktop ? (data.length > 6 ? 75 : 30) : 1, right: isDesktop ? 30 : 1, left: 20}}
      >
        ...
        {dataKeys.map((key, index) => <Bar layout={!isDesktop ? "vertical" : "horizontal"} key={key} dataKey={key}
                                           ...
                                           stackId="a"
                                           fill={customPalette ? customPalette[index % customPalette.length] : palette(index)}/>)}
        ))
        {
          isDesktop && usePercentage && haveAverageSuccessRate &&
          <Bar layout={!isDesktop ? "vertical" : "horizontal"} dataKey="avgSuccessRate" fill="#252525" barSize={5}/>
        }
        ...
        {isDesktop && <YAxis domain={[0, usePercentage ? 100 : parseDataMax]} scale={usePercentage ? 'linear' : scale}
                             tickFormatter={(value) => usePercentage ? `${value}%` : (currentDataTab === 'Count' ? value : convertBytesToIEC(value))}>
          <Label value={`${usePercentage ? '%' : '#'} ${String(unit).charAt(0).toUpperCase() + String(unit).slice(1)}s`}
                 position="left" offset={10} angle={270} fill="#666"/>
        </YAxis>}
        {!isDesktop && <YAxis dataKey="name" type="category" interval={0} minTickGap={0} stroke="#fff" mirror
                              tick={<CustomizedAxisTick/>}/>}
        {!isDesktop && <XAxis type="number" scale={usePercentage ? 'linear' : scale}
                              tickFormatter={(value) => usePercentage ? `${value}%` : (currentDataTab === 'Count' ? value : convertBytesToIEC(value))}
                              name="PiB" domain={[0, usePercentage ? 100 : parseDataMax]}>
          <Label value={`${usePercentage ? '%' : '#'} ${String(unit).charAt(0).toUpperCase() + String(unit).slice(1)}s`}
                 position="bottom" offset={-10} fill="#666"/>
        </XAxis>}
      </BarChart>
    </ResponsiveContainer>
  </div>
...
const CustomizedAxisTick = (props: {
  ...
  return (
    <g transform={`translate(${x},${y})`}>
      <text x={0} y={0} dx={isDesktop ? 5 : 0} dy={isDesktop ? 10 : 5} textAnchor="start"
