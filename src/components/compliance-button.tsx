...
const ComplianceDownloadButton = ({ id }: IProps) => {
  ...
  const generateComplianceReport = async () => {
    ...
    try {
      const apiResult = await fetch(
        `https://compliance.allocator.tech/report/${id}`
      );
      if (!apiResult.ok) {
        const result = await apiResult.json();
        throw new Error(
          result?.message || "Unable to generate compliance report"
        );
      }
  ...
  return (
    <div aria-disabled={complianceLoading}>
      <Button
        variant="outline"
      ...
      >
        {!complianceLoading && (
          <p className="flex items-center gap-2">
            Compliance Report <DownloadIcon className="md:hidden" size={15} />
          </p>
        )}
